...
export default function Page() {
  ...
  if (!DetailsCourses) return <div>Loading courses...</div>;

  const course = DetailsCourses.find(
    (c) => createSlug(c.course_title) === slug
  );
    ...
    <main>
          ...
          <div className="mt-36 lg:mr-80 lg:mt-0 lg:p-4 md:p-6">
            <div className="flex flex-col lg:flex-row text-white lg:ml-6">
              ...
              <div className="lg:mt-4 ml-10 mt-4">
                ...
                <p style={{ fontWeight: 300 }} className="my-5 text-lg">
            ...
            <div className="flex flex-col justify-between lg:flex-row text-[#17549A] mt-5 bg-white rounded-md ml-8 mr-8 lg:w-12/12 py-5">
              <div className="ml-5">
                ...
                <div className="gap-3 flex flex-col lg:flex-col items-start">
                  <div className="flex flex-col lg:flex-row gap-2">
                    <div className="flex items-center">
                      <SlCalender className="text-xs" />
                      <p>{course.startDate || "N/A"}</p>
                    </div>
                    <div className="flex items-center gap-1">
                      <HiBars4 className="text-sm" />
                      <h1>No. of Classes/Session: {course.numberOfClasses || "N/A"}</h1>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center">
                    <IoLocationOutline className="lg:mt-1" />
                    <p className="lg:ml-1">Venue: {course.venue || "N/A"}</p>
                  </div>

                  <div className="flex items-center">
                    <p className="ml-4">Contact: {course.contact || "N/A"}</p>
                    <FaArrowUpRightFromSquare className="ml-2 cursor-pointer text-blue-500" />
                  </div>
                </div>
              </div>
              ...
              <div className="lg:mt-4 lg:mr-20 mt-4 ml-5">
                <h2 className="text-2xl font-bold md:text-3xl">
                  Price: {course.course_cost || "N/A"}৳
                </h2>
                <p className="text-lg md:text-xl">{course.priceDetails || ""}</p>
      ...
      <div className="container mx-auto my-9 lg:w-9/12 w-10/12 ">
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          {/* Main Content: Left 4/5 */}
          <div className="w-full lg:w-4/5  shadow-lg p-4 rounded h-[1000px]">
            <div className="flex flex-col lg:flex-row gap-8 items-start">
              {/* Left Column: COURSE OUTLINE with fixed height */}
              <div className="w-full lg:w-3/5">
                <h2 className="text-center text-[#0054a5] font-semibold text-4xl mb-10">
                  COURSE OUTLINE
                </h2>

                {course.Course_Outlines?.map(({ title, desc }, index) => (
                  <ToggleBar
                    key={index}
                    id={index}
                    subject={title}
              ...
              {/* Right Column: Overview, Benefits, Projects */}
              <div className="w-full lg:w-2/5 lg:ml-8 ">

                {/* <hr className="w-20 h-1 bg-[#0054a6] mb-5" /> */}
                {/* {course.uploaded_video && (
                  <div className="w-full bg-gray-200 rounded-lg shadow-lg p-1 mb-8">
                    <video
                      controls
                      preload="metadata"
                      className="w-full h-auto max-h-[400px] rounded-lg"
                    >
                      <source src={course.uploaded_video} type="video/mp4" />
                      Your browser does not support the video tag.
                    </video>
                  </div>
                )} */}
                ...
                {/* COURSE BENEFITS */}
                <h3 className="text-[#0054a5] font-semibold text-2xl mb-2">
                  Course Benefits
                </h3>
                {/* <hr className="w-20 h-1 bg-[#0054a6] mb-5" /> */}
                <ul>
                  {course.Course_Benifits?.map((b, index) => (
                    <li key={index} className="flex items-center mt-2">
                      <TiArrowRight className="text-2xl mr-2" /> {b}
                    </li>
                  ))}
                </ul>

                {/* COURSE PROJECTS INCLUDE */}
                <h3 className="text-[#0054a5] font-semibold text-2xl mt-8 mb-2">
                  Course Projects Include
                </h3>
                <hr className="w-20 h-1 bg-[#0054a6] mb-5" />
                <ul>
                  {course.Course_Projects?.map((p, index) => (
                    <li key={index} className="flex items-center mt-2">
                      <TiArrowRight className="text-2xl mr-2" /> {p}
                    </li>
                  ))}
                </ul>
          ...
          {/* Sidebar: Right 1/5 */}
          <div className="w-full lg:w-1/5 bg-white rounded h-auto lg:h-[2000px]">
            {/* Instructor */}
            <div className="shadow-xl rounded py-10 mt-5">
              <div className="lg:ml-4">
                <p className="text-[#0054a5] font-semibold mt-3">MEET THE INSTRUCTOR</p>
                ...
                <div className="w-full h-full relative overflow-hidden group cursor-pointer">
                  <Image
                    ...
                    alt="instructor image"
                    src={course.instructor_image || instrastor}
                    className="w-full h-full object-cover transition-all duration-300 group-hover:filter-none filter grayscale"
                  />
                  <div className="bg-green-600 opacity-80 relative bottom-7 text-white font-bold">
            ...
            {/* Company / GIF / Who can join / Venue */}
            <div className="w-12/12 shadow-xl rounded p-4 mt-5">
              <Image width={400} height={400} alt="company image" src={img2} className="w-full h-full object-cover" />
            </div>

            <div className="shadow-xl rounded py-10 mt-5 text-center">
              <Image width={400} height={400} alt="gif file" src={course.uploaded_gif || gif1} />
              <p className="text-[#0054a5] mt-3">{course.gifText || "Build Career in Web"}</p>
            </div>

            {/* <div className="shadow-xl rounded py-10 mt-5 lg:ml-4"> */}
            {/* <p className="text-[#0054a5] font-semibold mt-3">Who Can Join</p> */}
            {/* <hr className="w-10 h-1 bg-[#0054a6]" /> */}
            {/* <p className="mt-4 text-base">{course.targetAudience}</p> */}
            {/* </div> */}
            ...
            <div className="shadow-xl rounded py-10 mt-5 lg:ml-4">
              <p className="text-[#0054a5] font-semibold mt-3">Training Venue</p>
              <hr className="w-10 h-1 bg-[#0054a6]" />
              <Image width={400} height={400} alt="company image" src={logo} className="relative right-3" />
              <p className="mt-4 text-base">{course.venue || "Venue details"}</p>
              <StarRatings
                rating={Number(course.star_rating) || 0}
                starDimension="20px"
                starSpacing="2px"
                starRatedColor="gold"
                numberOfStars={5}
