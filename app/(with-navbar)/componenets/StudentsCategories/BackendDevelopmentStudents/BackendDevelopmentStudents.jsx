// BackendDevelopmentStudents.js
import { useState, useEffect } from 'react';

import Link from 'next/link';
import Loader from '../../Loader/Loader';

const BackendDevelopmentStudents = ({ backend, searchQuery, currentPage, handlePageChange }) => {
  const itemsPerPage = 12;

  // Function to apply search and pagination for backend development students
  ...
  const { currentData, totalPages } = applySearchAndPagination(backend || []);
      ...
      <div className="container mx-auto w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 lg:mt-0">
        {backend.length === 0 ? (
          ...
          currentData.map((student) => (
            <div key={student.id}>
              <Link href={`/Students_Backend_Development/${student.id}`} className="relative gap-4 overflow-hidden cursor-pointer">
                <div className="lg:w-full">
                  <img src={student.image} className="w-full rounded-lg" alt="Backend Development" />
                </div>
...
export default BackendDevelopmentStudents;
