...
const GraphicsStudents = ({ graphics, searchQuery, currentPage, handlePageChange }) => {
  const itemsPerPage = 12;

  // Function to apply search and pagination for graphics students
  ...
  const { currentData, totalPages } = applySearchAndPagination(graphics || []);
            ...
            <div key={student.id}>
              <Link href={`/Students_Graphics/${student.id}`} className="relative gap-4 overflow-hidden cursor-pointer">
                <div className="lg:w-full">
                  <img src={student.image} className="w-full rounded-lg" alt="Graphics Design" />
                </div>
...
export default GraphicsStudents;
