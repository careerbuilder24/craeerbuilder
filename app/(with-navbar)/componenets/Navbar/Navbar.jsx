...
export default function Navbar() {
    ...
    const { user, logOut, ManualUser,logOutUserManual } = UserAuth();
    ...
    useEffect(() => {
        if (user && user.email) {
            const fetchUserProfile = async () => {
                try {
                    const response = await axios.get(`/api/user/profile?email=${user.email}`);
    ...
    // Add a log to check if userProfile is updating
    console.log("User Profile State:", ManualUser);
            ...
            <nav>
                                ...
                                <div className="relative group">
                                    <div className="text-center text-white hover:text-blue-400 duration-300 transition-all font-bold cursor-pointer whitespace-nowrap">
                                        <Link href="/Students" className="flex items-center justify-center h-full font-bold">
                                            Students
                                            <span className="ml-2 text-sm transition-transform duration-300 transform group-hover:rotate-180">&#x25BC;</span>
                                        </Link>
                                    </div>
                                    ...
                                    {ManualUser ? (
                                        <div className="flex flex-row items-center gap-4 cursor-pointer">
                                            <Image
                                                ...
                                                src={userProfile?.photo_url || ManualUser?.photoURL || logo3 || ''} // Ensure you have a fallback
                                                alt="User Profile"
                                            />
                                            <p className="text-white">{userProfile?.name || ManualUser?.name || 'Anonymous'}</p>
                                        </div>
                                    ) : ''}
                ...
                >
                    ...
                    <div className="flex flex-col items-center space-y-2 py-4 gap-3 ">
                        ...
                        <div style={{ borderRadius: '4px' }} className='h-10 w-[245px]  bg-white '>
                            <h3 className='ml-10 mt-1'>Career Guide</h3>
                        </div>
                        <div style={{ borderRadius: '4px' }} className='h-10 w-[245px]  bg-white '>
