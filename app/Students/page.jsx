...
function StudentGrid({ students, searchQuery, currentPage, handlePageChange, itemsPerPage = 12 }) {
      ...
      <div className="container mx-auto w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 lg:mt-0">
        {students.length === 0 ? (
          ...
          currentData.map((student) => {
            const nameSlug = slugifyName(student.name);
            return (
              <div key={student.id} className="relative overflow-hidden rounded-lg">
                <Link
                  href={`/profile/${nameSlug}-${student.id}`}
                  className="block"
                >
                  <img
...
export default function Page() {
  ...
  const studentsByCategory = useMemo(() => {
    ...
    const categorySlug = tabCategories[activeTabIndex].slug;
    if (categorySlug === 'all') return studentEditProfile.data;
    return studentEditProfile.data.filter((s) => s.category === categorySlug);
  }, [activeTabIndex, studentEditProfile]);
  ...
  return (
    <main>
      ...
      <div className="mt-28 bg-gray-100 h-full w-10/12 lg:w-7/12 container mx-auto mb-10">
        ...
        {/* Tabs */}
        <Tabs selectedIndex={activeTabIndex} onSelect={handleTabChange} className="flex flex-col lg:flex-row mt-4">
          {/* Tab List */}
          <TabList className="flex bg-[#0054a5] w-full md:w-5/12 lg:w-3/12 h-auto flex-col border-r border-gray-300 cursor-pointer hidden lg:flex sticky top-0 z-10 rounded-md">
            {tabCategories.map((tab, index) => (
              <Tab
                key={index}
                className={`p-4 text-left text-[#8dbff7] border-b border-[#DDDDDD] hover:bg-blue-200 hover:text-blue-700 whitespace-nowrap md:whitespace-normal ${
                  activeTabIndex === index ? 'bg-blue-200 text-blue-600' : ''
                }`}
              >
                {tab.name}
