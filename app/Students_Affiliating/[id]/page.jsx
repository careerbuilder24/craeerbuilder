...
import CuriculamVite from '@/app/(with-navbar)/componenets/RunningAffiliatingStudents/AffiliatingCv/AffiliatingCv';
...
import './Affiliating.css';

// ✅ Utility function for clean slug
const formatSlug = (text = '') => text.trim().replace(/\s+/g, '_').replace(/[^\w_]/g, '').toLowerCase();

export default function Affiliating() {
  const { id } = useParams();
  ...
  // Sync slug in URL whenever motion or tab changes
  useEffect(() => {
    if (!motion) return;
    const currentTab = searchParams.get("tab") || "profile";
    const slugName = formatSlug(motion.name || 'student');
    const emailQuery = MatchedEmail?.email ? `&email=${MatchedEmail.email}` : '';
    const newUrl = `/Students_Affiliating/${slugName}/${id}?tab=${currentTab}${emailQuery}`;
  ...
  const handleTabChange = (index) => {
    ...
    const slugName = formatSlug(motion.name || 'student');
    const emailQuery = MatchedEmail?.email ? `&email=${MatchedEmail.email}` : '';
    const newUrl = `/Students_Affiliating/${slugName}/${id}?tab=${tabSlug}${emailQuery}`;
    window.history.replaceState(null, '', newUrl);
  };
  ...
  return (
    <>
      <HelmetHead
        title={`${motion.name} - Affiliating Student`}
        description={`Details of Affiliating student ${motion.name}`}
        keywords="affiliating student, portfolio, courses, achievements"
        author="Muhibullah"
      />
      <Navbar />
