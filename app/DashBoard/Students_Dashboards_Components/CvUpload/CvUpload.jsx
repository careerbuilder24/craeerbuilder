...
export default function Page() {
  ...
  //           // Level + Year on same line
  //           eduText += `${edu.level || ""} ${edu.year || ""}\n`;

  //           // Institute on a new line
  //           if (edu.institute) {
  //             eduText += `${edu.institute}\n`;
  //           }

  //           // Major on a new line
  //           if (edu.major) {
  //             eduText += `Major: ${edu.major}\n`;
  //           }

  //           // Subject on a new line
  //           if (edu.subject) {
  //             eduText += `Subject: ${edu.subject}\n`;
  //           }

  //           // Board on a new line
  //           if (edu.board) {
  //             eduText += `Board: ${edu.board}\n`;
  //           }

  //           // Result on a new line
  //           if (edu.result) {
  //             eduText += `Result: ${edu.result}\n`;
  //           }

  //           // Extra info on a new line
  //           if (edu.extra) {
  //             eduText += `${edu.extra}\n`;
  //           }
  ...
  //           return eduText;
  //         })
  //         .join("\n") // Separate multiple education entries with a blank line
  //     },
      ...
      { title: "Objective", content: cvData.objective },
      {
        ...
        content: cvData.education
          .map((edu) => {
            ...
            if (edu.major) eduText += `Major: ${edu.major}\n`;
  ...
  const handleSubmit = async () => {
    ...
    const confirmation = await Swal.fire({
      ...
      reverseButtons: true, // swaps confirm and cancel positions
    });
    ...
    if (confirmation.isConfirmed) {
      try {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error!',
          text: 'Failed to submit CV. Please try again later.',
          confirmButtonColor: '#d33',
        });
      }
    } else {
      ...
      Swal.fire({
        icon: 'info',
        title: 'Cancelled',
        text: 'Your CV submission was cancelled.',
        timer: 1500,
