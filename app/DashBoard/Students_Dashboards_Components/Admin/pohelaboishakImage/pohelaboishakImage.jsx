...
export default function PohelaBoishakhImage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await axios.post('/api/pohela-boishakh', { imageUrl: uploadedUrl, title: title.trim() })
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`/api/pohela-boishakh?id=${id}`)
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      <h2 className="font-bold mb-6 text-center text-2xl"> Pohela Boishakh Images</h2>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="border rounded p-4 w-full md:w-1/3 order-1 md:order-2">
          ...
          <button
            ...
            disabled={uploading}
            className="bg-[#3B82F6] text-white px-4 py-2 rounded w-full disabled:opacity-50"
          >
        ...
        <div className="rounded p-4 overflow-y-auto max-h-[600px] flex-1 order-2 md:order-1">
          ...
          {images.length > 0 ? (
            ...
            <p className="text-sm text-gray-500">Uploaded Loading......</p>
          )}
        </div>
      </div>
