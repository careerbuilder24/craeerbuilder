...
// Helper: slugify name (spaces → underscores, remove symbols)
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const newUrl = `/Students-FrontEnd/${motion.id}/${nameSlug}?tab=${currentTab}`;
  ...
  const handleTabChange = (index) => {
    ...
    const nameSlug = slugifyName(motion.name);
    const newUrl = `/Students_FrontEnd/${motion.id}/${nameSlug}?tab=${slug}`;
    window.history.replaceState(null, '', newUrl);
  };
  ...
  return (
    <>
      <HelmetHead
        title={`${motion.name} - Motion Student`}
        description={`Details of Motion student ${motion.name}`}
