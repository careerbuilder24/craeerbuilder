...
import CuriculamVite from '@/app/(with-navbar)/componenets/RunningBackendStudents/RunningBackendStudentsCv/BackendCv';
...
import './BackendDevelopmnent.css';

// 🔹 Utility to make slug from name
...
export default function Page() {
  ...
  useEffect(() => {
    if (!motion) return;
    const nameSlug = slugifyName(motion.name);
    const currentTab = searchParams.get("tab") || "profile";
    const email = MatchedEmail?.email ? `&email=${MatchedEmail.email}` : "";
    const newUrl = `/Students-Backend-development/${id}/${nameSlug}?tab=${currentTab}${email}`;
    window.history.replaceState(null, '', newUrl);
  }, [MatchedEmail?.email, id, motion, searchParams]);
  ...
  const handleTabChange = (index) => {
    ...
    const nameSlug = slugifyName(motion?.name);
    const email = MatchedEmail?.email ? `&email=${MatchedEmail.email}` : "";
    const newUrl = `/Students-Backend-development/${id}/${nameSlug}?tab=${slug}${email}`;
    window.history.replaceState(null, '', newUrl);
  };
  ...
  return (
    <>
      <HelmetHead
        title={`${motion.title} - Motion Student`}
        description={`Details of Motion student ${motion.title}`}
