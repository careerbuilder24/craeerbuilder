...
export default function MotionStudentDetail() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    const newUrl = `/Students-Business-Development/${id}?tab=${currentTab}&email=${MatchedEmail.email}`;
  ...
  const handleTabChange = (index) => {
    ...
    const newUrl = `/Students-Business-Development/${id}?tab=${slug}${email ? `&email=${email}` : ''}`;
    window.history.replaceState(null, '', newUrl);
  };
