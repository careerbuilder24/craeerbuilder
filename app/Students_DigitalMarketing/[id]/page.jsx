...
export default function StudentDetailPage() {
  ...
  // Convert name → slug-friendly
  const formatNameToSlug = (name) => {
    if (!name) return '';
  ...
  // Update URL with name + tab
  const handleTabChange = (index) => {
    setActiveTabIndex(index);
    const slug = tabCategories[index].slug;
    const nameSlug = formatNameToSlug(student?.name);
    router.replace(`/Students_DigitalMarketing/${id}/${nameSlug}?tab=${slug}`, { scroll: false });
  };
                ...
                {tabCategories.map((category, index) => (
                  <li
                    ...
                    onClick={() => {
                      ...
                      setIsSidebarOpen(false);
                      const nameSlug = formatNameToSlug(student?.name);
                      router.replace(`/Students_DigitalMarketing/${id}/${nameSlug}?tab=${category.slug}`, { scroll: false });
                    }}
            ...
            <TabList className="hidden lg:flex flex-col border-r border-gray-300 bg-[#17549A] w-2/12 h-auto text-white">
              ...
              {tabCategories.map((tab, index) => (
                <Tab
                  ...
                  onClick={() => {
                    const nameSlug = formatNameToSlug(student?.name);
                    router.replace(`/Students_DigitalMarketing/${id}/${nameSlug}?tab=${tab.slug}`, { scroll: false });
                  }}
